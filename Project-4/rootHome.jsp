...
<body>
	...
	<p>You are connected to the Project 4 Enterprise System database as a root-level user.</p>
	...
	<div class = "container" id = "containerRes">
    	...
    	<!-- prints the table of a query made to a database -->
   		<%
			...
			Integer updatedRows = (Integer) request.getAttribute("rowsUpdated");
    	...
    	<%
        	} else if(commandType != null && commandType.equalsIgnoreCase("update") && updatedRows > 0) {
        		
        		Boolean hasBusinessLogicObj = (Boolean) request.getAttribute("hasBusinessLogic");
        		boolean hasBusinessLogic = hasBusinessLogicObj != null && hasBusinessLogicObj;
        		Integer changedStatuses = (Integer) request.getAttribute("increasedStats");
    	%>
        		<p>The statement executed successfully</p>
        		<p><%= updatedRows %> row(s) affected.</p>
   			<%
       			if (hasBusinessLogic == true){
       				
       			%>
       				<p>Business Logic Detected! - Updating Supplier Status</p>
       				<p>Business Logic updated <%= changedStatuses %> supplier status marks</p>
       			<%
       			}else{
       				
       			%>
       				<p>Business Logic Not Triggered</p>
       			<%
       			}
        	%>
    			
    	<%
        	} else if (commandType != null && commandType.equalsIgnoreCase("insert") && updatedRows > 0) {
    			
        		Boolean hasBusinessLogicObj = (Boolean) request.getAttribute("hasBusinessLogic");
        		boolean hasBusinessLogic = hasBusinessLogicObj != null && hasBusinessLogicObj;
    	%>
        		<p>The statement inserted successfully</p>
        		<p><%= updatedRows %> row(s) inserted.</p>
        	<%
       			if (hasBusinessLogic == true){
       				
       			%>
       				<p>Business Logic Detected! - Updating Supplier Status</p>
       				<p>Business Logic updated 1 supplier status marks</p>
       			<%
       			}else{
    	...
    	<%
        		...
        		%>
        			<p>Error executing the SQL statements:</p>
        			<p><%= errorReason %></p>
        		<%
        		} else { 
    	%>
